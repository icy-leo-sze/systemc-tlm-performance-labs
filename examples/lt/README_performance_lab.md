...
Project D standalone C++ trace replay engine、Project E banked memory controller
queueing model、Project F gem5 stats trend correlation report。
...
- Project E 会把 replay model 进一步推进到 standalone C++ banked memory controller
  + queueing abstraction，观察 bank pressure、row-buffer locality、queue occupancy、
  tail latency、bank utilization 和 rejected transaction 趋势。
- Project F 会把 gem5 SE `stats.txt` 与 replay / Project E summary join 成
  qualitative trend-level correlation report，用于解释 `sequential` vs `stride`
  趋势是否一致。
...
Project E banked memory controller queueing MVP 使用独立 C++ binary 承担核心模型逻辑。
...
Project E 第一版不接 SystemC kernel，不接 gem5 live co-simulation，不做 JEDEC DRAM
timing，不实现 AXI、CHI 或 NoC protocol，也不声称 cycle accuracy。

Project F gem5 stats trend correlation MVP 使用独立 demo wrapper。它要求默认路径下已
存在 Project C gem5 SE `stats.txt`、replay `summary.csv` 和 Project E `summary.csv`：

```bash
python3 examples/lt/tools/demo_gem5_stats_correlation_lab.py
```

Project F 只做 file-based qualitative / trend-level report。缺少非必需字段时会写
`NA` 并在 `trend_notes` 说明；缺少输入文件时会提示先运行对应 demo。它不做 gem5 live
co-simulation，不声称 cycle accuracy，也不声称 RTL / silicon / profiler correlation。
...
column -s, -t examples/lt/results/project_e_banked_memory_controller/summary.csv
sed -n '1,220p' examples/lt/results/project_e_banked_memory_controller/comparison.md
column -s, -t examples/lt/results/project_f_gem5_stats_correlation/correlation_summary.csv
sed -n '1,220p' examples/lt/results/project_f_gem5_stats_correlation/correlation_report.md
```
...
- `examples/lt/results/cpp_trace_replay_lab/comparison.md`：Project D demo 从 C++
  `summary.csv` 生成的对比说明。
- `examples/lt/results/project_e_banked_memory_controller/trace.csv`：Project E
  standalone C++ banked memory controller model 生成的 per-transaction trace。
- `examples/lt/results/project_e_banked_memory_controller/summary.csv`：Project E
  一行一个 workload 的 summary，至少包含 bank/queue/tail latency/utilization/row-hit/reject
  关键字段。
- `examples/lt/results/project_e_banked_memory_controller/comparison.md`：Project E demo
  从 C++ `summary.csv` 生成的 `sequential_scan` / `stride_scan` / `hot_bank_stress`
  对比说明。
- `examples/lt/results/project_f_gem5_stats_correlation/correlation_summary.csv`：
  Project F 从 gem5 `stats.txt`、replay summary 和 Project E summary join 得到的
  trend correlation summary。
- `examples/lt/results/project_f_gem5_stats_correlation/correlation_report.md`：
  Project F 生成的 qualitative / trend-level correlation report，解释
  `sequential` vs `stride` 趋势、gem5 stats 的外部参照作用，以及 `timestamp_ns`
  为什么不是 gem5 timing。
...
| Project E banked memory controller | `sequential_scan p99 = 60.000 ns`，`stride_scan p99 = 424.000 ns`，`hot_bank_stress p99 = 960.000 ns`，`hot_bank_stress stalled_or_rejected_transactions = 68` |
| Project F gem5 stats correlation | 生成 `correlation_summary.csv` 和 `correlation_report.md`，只说明 qualitative / trend-level correlation，不比较绝对 cycle |
...
| Project E | standalone C++ banked memory controller queueing MVP |
...
- 把 Project D 的 toy bank-conflict replay 进一步升级为 standalone C++ banked memory
  controller + queueing model。
- 输入仍然复用 normalized trace CSV。
- 输出仍然保持 `trace.csv`、`summary.csv` 和 `comparison.md`。
- C++ 承担 bank mapping、queueing、row-buffer、latency、utilization、throughput 和 reject
  统计。
- Python 只负责 demo orchestration、demo input trace generation 和 `comparison.md`
  generation。
...
| workload | 建模含义 |
| --- | --- |
| `sequential_scan` | 连续 4-byte access，多 bank 轮转，latency tail 较稳 |
| `stride_scan` | 16-byte stride，制造 bank pressure 和 row-boundary 行为差异 |
| `hot_bank_stress` | 短 issue spacing + hot bank + row miss，推高 queue occupancy、p99/max latency 和 reject 风险 |

Project E `summary.csv` 至少包含：
...
```text
docs/project_e_banked_memory_controller_report.md
```

非目标声明：

- Project E 不接 SystemC kernel。
- Project E 不做 gem5 live co-simulation。
- Project E 不做 JEDEC DRAM timing。
- Project E 不实现 AXI、CHI 或 NoC protocol。
- Project E 不声称 cycle accuracy。
- Project E 当前证明的是 trend-level memory subsystem behavior，不是
  cycle-accurate DRAM。
