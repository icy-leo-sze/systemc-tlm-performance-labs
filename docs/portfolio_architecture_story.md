...
analysis，再到 cache-like shared-resource / MSHR pressure analysis，以及
memory-system backpressure / QoS collapse analysis。项目目标不是
...
pressure、shared interference 和 memory-service bottleneck，Project AT-5 关注
bounded queues、downstream saturation、backpressure propagation 和 QoS collapse。
当前模型适合表达趋势、瓶颈、policy sensitivity 和 bounded recommendation，不适合表达真实芯片验证或生产级协议签核。
...
| AT backpressure / QoS collapse | Project AT-5 | AT / downstream bottleneck isolation | When do bounded queues and downstream saturation make QoS policies lose effectiveness? | queue_full_events, backpressure_stall_ns, service_utilization, saturation_ratio, collapse_score | summary, policy sweep, recommendations, report | bounded AT-level bottleneck isolation；不是 real NoC、AXI/CHI、DRAM controller、cache coherence、cycle accuracy 或 silicon validation |
...
单独分析。对于 Project AT-1/2/3/4/5，AT 更接近早期 SoC architecture exploration 里对
仲裁、排队、tail latency、SLA risk、cache-like shared-resource pressure 和
downstream saturation 的讨论方式。
...
7. Project AT-5: priority is not magic; if the bottleneck is downstream service
   capacity, QoS can only redistribute pain. 在 `downstream_saturation_qos_collapse`
   这类 case 中，priority policy 可以改变谁先等、谁后等，但不能创造新的服务能力。
...
- memory-system backpressure and QoS collapse reasoning
- downstream saturation vs policy tuning judgment
...
Project AT-5 的 claim boundary 是：它只支持 AT-level bounded bottleneck isolation
和 trend comparison，用来讨论 bounded queues、downstream service saturation、
backpressure propagation 和 QoS policy limit；它不是 real NoC、AXI/CHI、DRAM
controller、cache coherence、cycle-accurate 或 silicon validation model。
...
traffic interference without claiming real cache coherence. Project AT-5 then
adds a backpressure and QoS collapse lab: it shows that priority can redistribute
contention, but when downstream service capacity is saturated, QoS policy alone
...
5. Run Project AT-5 demo.
6. Inspect Project AT-5 recommendations.
7. Backtrack to Project AT-4 for shared-resource / MSHR-like pressure.
8. Backtrack to Project AT-3 for QoS/SLA.
9. Backtrack to Project AT-2 for arbitration.
10. Backtrack to AT-1 for four-phase timing.
11. Backtrack to K/L for LT bottleneck and recommendation logic.
